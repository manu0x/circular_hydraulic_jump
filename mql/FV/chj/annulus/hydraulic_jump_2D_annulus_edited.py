...
def step_diffuse_rk4(solver,state,dt):

    
    vs = state.problem_data['viscosity']
    ...
    dr = state.grid.r.delta
    dtheta = state.grid.theta.delta
   ...
   #     print("mu ALERT   ",mu)
    ...
    h_rr = (-2.0*h+hbc[num_ghost+1:-num_ghost+1,num_ghost:-num_ghost]+hbc[num_ghost-1:-num_ghost-1,num_ghost:-num_ghost])
    h_r = (hbc[num_ghost+1:-num_ghost+1,num_ghost:-num_ghost]-hbc[num_ghost-1:-num_ghost-1,num_ghost:-num_ghost])
    h_r = h_r/(2.0*dr)
    h_rr = h_rr/(dr*dr)

    h_oo = (-2.0*h+hbc[num_ghost:-num_ghost,num_ghost+1:-num_ghost+1]+hbc[num_ghost:-num_ghost,num_ghost-1:-num_ghost-1])
    h_oo = h_oo/(dtheta*dtheta)


    lap_h = h_r/(rc) + h_rr + h_oo/(rc*rc)

    hk1 =  dt*vs*lap_h
    hk =   h + 0.5*hk1
    

    hbc[num_ghost:-num_ghost,num_ghost:-num_ghost] = hk

    h_rr = (-2.0*hk+hbc[num_ghost+1:-num_ghost+1,num_ghost:-num_ghost]+hbc[num_ghost-1:-num_ghost-1,num_ghost:-num_ghost])
    h_r = (hbc[num_ghost+1:-num_ghost+1,num_ghost:-num_ghost]-hbc[num_ghost-1:-num_ghost-1,num_ghost:-num_ghost])
    h_r = h_r/(2.0*dr)
    h_rr = h_rr/(dr*dr)
    ...
    state.q[0,:,:] = state.q[0,:,:] + (1.0/6.0)*(hk1+2.0*hk2+2.0*hk3+hk4)
...
def setup(
    ...
    if diff_vis:
        solver.step_source = step_diffuse_rk4
        solver.source_split = 1
    ...
    if restart_from_frame is not None:
        ...
        state.problem_data['viscosity'] = viscosity
